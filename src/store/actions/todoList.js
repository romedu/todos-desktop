	...
	return dispatch => {
		qwest
			...
			.then(response => {
				...
				if (status && status !== 200) throw new Error(message);
				if (listFolder) {
					if (editedList.container === listFolder) return dispatch(updateFile(editedList));
					if (!editedList.container) return dispatch(removeFile(editedList._id));
					return dispatch(moveFile(editedList));
				}
				//CALLED WHEN CHANGED FROM NO FOLDER TO FOLDER
				else if (editedList.container) return dispatch(removeList(editedList._id));
