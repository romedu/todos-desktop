...
export const getFolders = (sortProp, sortOrder, pageParam) => {
	...
	return dispatch => {
		qwest
			.get(`/api/folder?sortProp=${sortProp}&sortOrder=${sortOrder}&page=${pageParam}&limit=14`, null, { headers })
...
export const openFolder = folderId => {
	...
	return dispatch => {
		qwest
			.get(`/api/folder/${folderId}`, null, { headers })
			.then(data => JSON.parse(data.response))
			.then(response => {
...
export const createFolder = newFolderData => {
	...
	return dispatch => {
		qwest
			.post("/api/folder", newFolderData, { headers })
			.then(data => JSON.parse(data.response))
			.then(response => {
...
export const updateFolder = (folderId, payload) => {
	...
	return dispatch => {
		qwest
			.map("PATCH", `/api/folder/${folderId}`, payload, { headers })
			.then(data => JSON.parse(data.response))
			.then(response => {
